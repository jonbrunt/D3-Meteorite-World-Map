...
function main() {
  ...
  const projection = d3
    .geoEquirectangular() // map projection type
    .translate([600, 311])
    .scale(191);
  const path = d3.geoPath().projection(projection);
  // map data/path execution
  map.append("path").attr("d", path(worldPath)); // JSON object stored in separate JS file
  ...
  d3.json(url, (err, obj) => {
    ...
    const data = rawData.filter(
      d => d.geometry && parseInt(d.properties.mass, 10) > 0
    );
